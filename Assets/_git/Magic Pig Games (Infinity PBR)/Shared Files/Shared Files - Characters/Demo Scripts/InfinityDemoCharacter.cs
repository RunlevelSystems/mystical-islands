...
namespace InfinityPBR.Demo
{
    public class InfinityDemoCharacter : MonoBehaviour
    {
        ...
        [Header("Options")]
    #if ENABLE_INPUT_SYSTEM
        public Key superRandomKey = Key.R;
    #else
        ...
        public virtual void Start()
        {
            ...
            _transform = GetComponent<Transform>(); _startPosition = _transform.localPosition;
        ...
        public void Update()
        {
            ...
            bool rightPressed = keyboard != null && keyboard.rightArrowKey.wasPressedThisFrame;
            bool leftPressed = keyboard != null && keyboard.leftArrowKey.wasPressedThisFrame;
            bool spacePressed = keyboard != null && keyboard.spaceKey.wasPressedThisFrame;
            bool superPressed = keyboard != null && keyboard[superRandomKey].wasPressedThisFrame;
#else
            bool rightPressed = Input.GetKeyDown(KeyCode.RightArrow);
            bool leftPressed = Input.GetKeyDown(KeyCode.LeftArrow);
            bool spacePressed = Input.GetKeyDown(KeyCode.Space);
            bool superPressed = Input.GetKeyDown(superRandomKey);
#endif

            if (rightPressed) SetAnimation(_animationTriggerIndex += 1);
            if (leftPressed)  SetAnimation(_animationTriggerIndex -= 1);
            if (spacePressed) TriggerAnimation();
            if (superPressed && !ShiftIsDown())
            {
        ...
        protected virtual void CheckPlayArea()
        {
            if (playArea == null)
            {
                var go = GameObject.Find("Play Area");
                if (go != null) playArea = go.GetComponent<Collider>();
        ...
        {
            Debug.LogWarning($"[ResetPositionAndRotation] <b>{name}</b> is outside play area — resetting.", this);
            ResetPosition();
            ResetRotation();
        }
