using UnityEngine;
#if ENABLE_INPUT_SYSTEM
using UnityEngine.InputSystem;
#endif

namespace InfinityPBR
{
    public class ExtendedFlycam : MonoBehaviour
    {
        public float CameraSensitivity = 90;
        public float NormalMoveSpeed = 10;
        public float SlowMoveFactor = 0.25f;
        public float FastMoveFactor = 3;

        // Converts raw mouse pixel delta to the legacy Input.GetAxis range at default Unity sensitivity.
        private const float MouseDeltaScale = 0.1f;

        private float _rotationX;
        private float _rotationY;

        // ReSharper disable once UnusedMember.Local
        private void Start()
        {
            _rotationX = transform.eulerAngles.y;
        }
        // ReSharper disable once UnusedMember.Local
        private void Update()
        {
#if ENABLE_INPUT_SYSTEM
            var keyboard = Keyboard.current;
            var mouse    = Mouse.current;

            if (mouse != null && mouse.rightButton.isPressed)
            {
                var delta = mouse.delta.ReadValue();
                _rotationX += delta.x * MouseDeltaScale * CameraSensitivity * Time.deltaTime;
                _rotationY += delta.y * MouseDeltaScale * CameraSensitivity * Time.deltaTime;
                _rotationY  = Mathf.Clamp(_rotationY, -90f, 90f);
            }

            var targetRotation  = Quaternion.AngleAxis(_rotationX, Vector3.up);
            targetRotation     *= Quaternion.AngleAxis(_rotationY, Vector3.left);
            transform.rotation  = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * 4f);

            if (keyboard == null) return;

            float speedFactor = 1f;
            if (keyboard.leftShiftKey.isPressed || keyboard.rightShiftKey.isPressed) speedFactor = FastMoveFactor;
            if (keyboard.leftCtrlKey.isPressed  || keyboard.rightCtrlKey.isPressed)  speedFactor = SlowMoveFactor;

            float vertical   = (keyboard.wKey.isPressed ? 1f : 0f) + (keyboard.sKey.isPressed ? -1f : 0f);
            float horizontal = (keyboard.dKey.isPressed ? 1f : 0f) + (keyboard.aKey.isPressed ? -1f : 0f);

            transform.position += transform.forward * NormalMoveSpeed * speedFactor * vertical   * Time.deltaTime;
            transform.position += transform.right   * NormalMoveSpeed * speedFactor * horizontal * Time.deltaTime;

            float upAxis = 0f;
            if (keyboard.qKey.isPressed) upAxis = -0.5f;
            if (keyboard.eKey.isPressed) upAxis =  0.5f;
            transform.position += transform.up * NormalMoveSpeed * speedFactor * upAxis * Time.deltaTime;
#else
            if (Input.GetMouseButton(1)) // LEGACY - TO REMOVE
            {
                _rotationX += Input.GetAxis("Mouse X") * CameraSensitivity * Time.deltaTime;
                _rotationY += Input.GetAxis("Mouse Y") * CameraSensitivity * Time.deltaTime;
                _rotationY = Mathf.Clamp(_rotationY, -90, 90);
            }

            Quaternion targetRotation = Quaternion.AngleAxis(_rotationX, Vector3.up);
            targetRotation *= Quaternion.AngleAxis(_rotationY, Vector3.left);
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * 4f);

            float speedFactor = 1f;
            if (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)) speedFactor = FastMoveFactor;
            if (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)) speedFactor = SlowMoveFactor;

            transform.position += transform.forward * NormalMoveSpeed * speedFactor * Input.GetAxis("Vertical") *
                                  Time.deltaTime;
            transform.position += transform.right * NormalMoveSpeed * speedFactor * Input.GetAxis("Horizontal") *
                                  Time.deltaTime;
            float upAxis = 0;
            if (Input.GetKey(KeyCode.Q)) upAxis = -0.5f;
            if (Input.GetKey(KeyCode.E)) upAxis = 0.5f;
            transform.position += transform.up * NormalMoveSpeed * speedFactor * upAxis * Time.deltaTime;
        }
#endif
    }
}